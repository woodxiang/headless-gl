...
class WebGLRenderingContext extends NativeWebGLRenderingContext {
  ...
  bindFramebuffer(target, framebuffer) {
    ...
    if (!framebuffer) {
      super.bindFramebuffer(gl.FRAMEBUFFER, this._drawingBuffer._framebuffer);
    } else if (framebuffer._pendingDelete) {
      return;
    } else if (this._checkWrapper(framebuffer, WebGLFramebuffer)) {
      super.bindFramebuffer(gl.FRAMEBUFFER, framebuffer._ | 0);
    } else {
      return;
    }
  ...
  readPixels(x, y, width, height, format, type, pixels) {
    ...
    if (x >= viewWidth || x + width <= 0 || y >= viewHeight || y + height <= 0) {
      ...
      if (nWidth > 0 && nHeight > 0) {
        ...
        super.readPixels(nx, ny, nWidth, nHeight, format, type, subPixels);
      ...
      super.readPixels(x, y, width, height, format, type, pixelData);
    }
  }
